...
void gemm_large_n_run(uint32_t iter) {
    ...
    //GEMM input size
    // uint32_t matrix_m = 115140;
    // uint32_t matrix_n = 50404;
    // uint32_t matrix_k = 4096;
    uint32_t matrix_m = 4096;
    uint32_t matrix_n = 40960;
    uint32_t matrix_k = 4096;
    ...
    using dispatch_policy = xetla::kernel::dispatch_policy_persistent_thread<8>;

    using gemm_op_t
            = xetla::kernel::gemm_t<dispatch_policy, brgemm_config, epilogue_t>;

    cl::sycl::nd_range<3> nd_range
            = gemm_op_t::get_nd_range(matrix_m, matrix_n);
    ...
    for (uint32_t i = 0; i < iter + warmup; i++) {
        ...
        auto gpu_event = queue.submit([&](handler &cgh) {
            ...
            cgh.parallel_for(nd_range, [=](nd_item<3> item) SYCL_ESIMD_KERNEL {
                ...
                typename gemm_op_t::arguments_t gemm_arg(matrix_m, matrix_k,
