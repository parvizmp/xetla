...
//fp4(1-3-0, bias1) => fp16
//#if (XETLA_CODE_BASE == __ESIMD__)
//using INT16V16 = uint16_t __attribute__((ext_vector_type(16)));
//const INT16V16 fp4_fp16_table = {0x0000, 0x3c00, 0x4000, 0x4400, 0x4800, 0x4c00,
//        0x5000, 0x5400, 0x0000 /*0x7fff*/, 0xbc00, 0xc000, 0xc400, 0xc800,
//        0xcc00, 0xd000, 0xd400};
//#elif (XETLA_CODE_BASE == __CM__)
//const uint16_t fp4_fp16_table[16] = {0x0000, 0x3c00, 0x4000, 0x4400, 0x4800,
//        0x4c00, 0x5000, 0x5400, 0x0000 /*0x7fff*/, 0xbc00, 0xc000, 0xc400,
//        0xc800, 0xcc00, 0xd000, 0xd400};
//#endif
...
template <typename compute_attr_, typename perf_tuning_knob_,
        ...
        int dequant_s_, typename pre_processing_t_>
class brgemm_t<compute_policy_dequantize_matB_xmx<compute_attr_,
                       perf_tuning_knob_, dtype_scale_, dtype_zero_pt_,
                       dtype_table_, dequant_s_, gpu_arch::Xe>,
...
public:
    ...
    using compute_policy = compute_policy_dequantize_matB_xmx<compute_attr_,
            perf_tuning_knob_, dtype_scale_, dtype_zero_pt_, dtype_table_,
            dequant_s_, gpu_arch::Xe>;
...
private:
    ...
    using dtype_table = typename compute_policy::dtype_table;

    static_assert(std::is_same<remove_const_t<dtype_b>,
                          remove_const_t<bit4x2>>::value,
    ...
    static_assert(!is_local_b, "only support MatB from global for now");
    ...
    using matB_tile_desc_t
            = subgroup::tile_desc_t<tile_size_x_b / 2, tile_size_y_b,
                    block_size_x_b / 2, block_size_y_b, reg_layout::tiled>;
...
private:
    ...
    using zero_pt_tile_desc_t = subgroup::tile_desc_t<tile_size_x_b / 2,
            tile_size_y_zero_pt, block_size_x_b / 2, block_size_y_zero_pt,
            reg_layout::tiled>;
    ...
    using table_tile_desc_t
            = subgroup::tile_desc_t<16, 1, 16, 1, reg_layout::tiled>;
    using table_t = subgroup::tile_t<dtype_table, table_tile_desc_t>;
    using table_payload_t = subgroup::mem_payload_t<dtype_table,
            table_tile_desc_t, msg_type::block_1d, mem_layout::row_major,
...
public:
    ...
    struct arguments_t {
        ...
        //scale size: (matrix_k/dequant_block_size_y)x(matrix_n)
        mem_desc_scale_t scale_base_desc;
        //zero_pt size: (matrix_k/dequant_block_size_y)x(matrix_n/2)
        mem_desc_zero_pt_t zero_pt_base_desc;
        mem_desc_table_t table_base_desc;
        ...
        inline arguments_t(mem_desc_a_t matA_desc, mem_desc_b_t matB_desc,
                ...
                mem_desc_zero_pt_t zero_pt_desc, mem_desc_table_t table_desc)
        ...
        inline arguments_t(const arguments_t &args)
            ...
            , table_base_desc(args.table_base_desc) {}
        inline arguments_t &operator=(const arguments_t &args) {
            ...
            this->table_base_desc = args.table_base_desc;
            return *this;
        }
    ...
    __XETLA_API KERNEL_FUNC void operator()(work_group_t &g, matAcc_t &matAcc,
        ...
        // table_payload_t table_payload(args.table_base_desc);
...
private:
    inline void dequantize(matB_acc_t &matB_acc, matB_t &matB, scale_t &scale,
        ...
        // constexpr uint32_t vnni_blks = block_size_y_b / vnni_rows;
        constexpr uint32_t block_b_y_per_scale = dequant_s / block_size_y_b;
        static_assert(block_size_x_b == 2 * matB_t::block_size_x, "for bit4x2");
#pragma unroll
...
#pragma unroll
            for (int j = 0; j < num_block_x; ++j) {
                ...
                auto scale_vec = scale.reg.xetla_select<block_size_x_b, 1>(
                        scale_block_id * block_size_x_b);
                auto zero_pt_vec
                        = zero_pt.reg
                                  .xetla_select<block_size_x_b / 2, 1>(
                                          scale_block_id * block_size_x_b / 2)
                                  .xetla_format<uint8_t>();
...
#pragma unroll
    ...
    __XETLA_API static void update_sg_tile_tdesc(
        ...
        args.matB_base_desc.update_coord_x(tile_offset_n / 2);
        args.scale_base_desc.update_coord_x(tile_offset_n);
        args.zero_pt_base_desc.update_coord_x(tile_offset_n / 2);
    }
};
